...
![](\images\posts\Shadertoy\shadertoy_snake1.gif)
...
float RayMarch(vec3 ro, vec3 rd) {
    ...
    for(int i=0; i<MAX_STEPS; i++) {
    	vec3 p = ro + rd*dO;
        float dS = GetDist(p); // 得到当前最远能步进的距离
        dO += dS;
...
vec3 Render(vec2 uv, vec2 m, float t) {
    // Ray
    vec3 ro = vec3(0, 0, -3)*(8.+sin(t*.2)*2.*CAM_MOVE); // Ray当前起点位置
    ro.yz *= Rot(-m.y*3.14+sin(t*.03)*CAM_MOVE*.2); // Ray绕横轴旋转
    ro.xz *= Rot(-m.x*6.2831*2.+sin(t*.05)*CAM_MOVE); // Ray绕纵轴旋转
    
    vec3 rd = GetRayDir(uv, ro, vec3(0,0,sin(t*.11)), 6.); // 得到Ray的方向
    
    float d = RayMarch(ro, rd); // Raymarching，得到当前最远能步进的距离d
    
    vec3 col = vec3(0);
    
    vec2 env = RaySphere(ro, rd, vec4(0,0,0,20)); // 
    
    if(d<MAX_DIST) { // 说明光线碰到了物体（snake）
        ...
        if(env.y>0.)	// vines behind 藤蔓在后面时
            col *= S(0., 1.1, sdGyroid(ro + env.y*rd, .4, .1, .0))*.5+.5;
    }
    
    if(env.x>0.)	// vines in front 藤蔓在前面时
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 uv = (fragCoord-.5*iResolution.xy)/iResolution.y; // 这一行代码等于下面三行代码
	...
	vec2 m = (iMouse.xy-.5*iResolution.xy)/iResolution.xy; // 这一行代码等于下面两行代码
    // vec2 uv = iMouse.xy / iResolution.xy; // 屏幕空间归一化至0——1范围。
    // uv -= 0.5; // 从0——1范围变为-0.5——0.5范围，坐标(0,0)由左下角移到矩形中心，便于后面绘制。
    if(m.x<-.49 && m.y<-.49) m*=0.;
    
    vec3 col = Render(uv, m, iTime); // 整个场景渲染
